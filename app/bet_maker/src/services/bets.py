...
class BetService(BaseService):
    ...
    async def create_by_event(self, client_id: int, event_id: int, **kwargs) -> BetResponseSchema | None:
        ...
        if bet is not None:
            raise HTTPException(status_code=HTTP_400_BAD_REQUEST, detail=f'You have already bet on event {event_id}')
        event = await EventService(self.db_session).retrieve(id=event_id)
        if event is None:
            raise HTTPException(status_code=HTTP_400_BAD_REQUEST, detail=f'Unknown event {event_id}')
        ...
        async with self.db_session as session:
            ...
            for event in events:
                ...
                for m in res.scalars().all():
                    ...
                    if schema.state == BetState.unknown.name:
                        continue
